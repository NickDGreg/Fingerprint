...
def load_asn_db(db_path: str | None) -> tuple[Any | None, str | None]:
    if not db_path:
...
def lookup_asn(db: Any | None, ip_addresses: list[str]) -> str | None:
...
def jarm_runtime_error() -> str | None:
    if not jarm_scanner:
...
def format_x509_name(name: Any) -> str | None:
    if not name:
        return None
    parts: list[str] = []
    for entry in name:
...
def collect_tls_info(hostname: str, timeout_ms: int) -> dict[str, object]:
    if not hostname:
    ...
    try:
        ...
        with socket.create_connection(
            ...
            with context.wrap_socket(sock, server_hostname=hostname) as tls_sock:
                ...
                return {
                    "cert_sha1": sha1 or "",
                    "cert_sha256": sha256 or "",
                    "cert_subject": format_x509_name(cert.get("subject")) or "",
                    "cert_issuer": format_x509_name(cert.get("issuer")) or "",
                    "cert_not_before": cert.get("notBefore") or "",
                    "cert_not_after": cert.get("notAfter") or "",
                }
    except Exception as error:
        return {
